Install in Foundry VTT via manifest:
```https://raw.githubusercontent.com/mdf-irl/crispy-critters/refs/heads/master/module.json```

# Attribution

## Code
All code in this module is licensed under the MIT License.

See `/LICENSE.md` for full text.

## Artwork / Textures
All dice textures, images, and visual assets are:

Copyright (c) 2026 mdf

Licensed under: Creative Commons Attribution-NonCommercial 4.0 (CC BY-NC 4.0)

You are free to:
* Share and adapt with attribution

Restrictions:
* Do not redistribute without proper credit
* Commercial use is prohibited

## Fonts
This module includes fonts licensed under the SIL Open Font License (OFL).

### New Rocker
Copyright (c) 2011, Pablo Impallari (www.impallari.com / impallari@gmail.com),

Copyright (c) 2011, Brenda Gallo (gbrenda1987@gmail.com),

Copyright (c) 2011, Rodrigo Fuenzalida (www.rfuenzalida.com / hello@rfuenzalida.com), 

with Reserved Font Name 'New Rocker'

* Source: https://fonts.google.com/specimen/New+Rocker

This font software is licensed under the SIL Open Font License, Version 1.1.

This license is included in `/fonts/new-rocker-LICENSE.txt`.
